...
export default function DashboardClient({ user, profile }: { user: User; profile: Profile | null }) {
  ...
  const handleSignOut = async () => {
    try {
      ...
      router.replace('/auth?mode=login&amp;redirect=/dashboard');
    } catch (err) {
                  ...
                  <div key={project.id} className="relative group rounded-2xl overflow-hidden border border-white/10 bg-white/5">
                    <div className="aspect-[4/3] w-full h-auto">
                      {project.generated_image_url ? (
                        <img src={project.generated_image_url} alt={project.name} className="w-full h-full object-cover" />
                      ) : project.primary_image_url ? (
                        <img src={project.primary_image_url} alt={project.name} className="w-full h-full object-cover" />
                      ) : (
